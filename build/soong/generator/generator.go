...
func init() {
	android.RegisterModuleType("corpora_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = corporaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = corporaExpandVariables(ctx, dep_file)
	...
	cmd := corporaExpandVariables(ctx, String(g.properties.Cmd))
